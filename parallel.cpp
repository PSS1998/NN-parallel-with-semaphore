...
void *hidden_calculation(void *threadarg){
    sem_wait(&lock_out_pass);
    struct hidden_thread_data *my_data;
    struct hidden_thread_data my_data2;
    my_data = (struct hidden_thread_data *) threadarg;
    my_data2.img = my_data->img;
    my_data2.thread_id = my_data->thread_id;
    free(threadarg);
    sem_post(&lock_out_pass);


    for (int j = 0; j < NUMBER_OF_HIDDEN_CELLS/num_hidden_threads; j++) {

        int* index_j = (int*)malloc(sizeof(int));
        *index_j = j+((NUMBER_OF_HIDDEN_CELLS/num_hidden_threads)*(my_data2.thread_id));
        hidden_nodes[*index_j].output = 0;
        for (int z = 0; z < NUMBER_OF_INPUT_CELLS; z++) {
            hidden_nodes[*index_j].output += my_data2.img->pixel[z] * hidden_nodes[*index_j].weights[z];
        }



        hidden_nodes[*index_j].output += hidden_nodes[*index_j].bias;
        hidden_nodes[*index_j].output = (hidden_nodes[*index_j].output >= 0) ?  hidden_nodes[*index_j].output : 0;
...
void *output_calculation(void *threadarg){
    struct out_thread_data *my_data;
    struct out_thread_data my_data2;
    sem_wait(&lock_pass);
    my_data = (struct out_thread_data *) threadarg;
    my_data2.img = my_data->img;
    my_data2.thread_id = my_data->thread_id;
    free(threadarg);
    sem_post(&lock_pass);

    for (int i= 0; i < NUMBER_OF_OUTPUT_CELLS/10; i++){
        int index_i = i+((NUMBER_OF_OUTPUT_CELLS/10)*(my_data2.thread_id));
        struct hidden_thread_data td_hidden[num_hidden_threads];
        output_nodes[index_i].output = 0;


        for (int j = 0; j < NUMBER_OF_HIDDEN_CELLS; j++){
            output_nodes[index_i].output += hidden_nodes[j].output * output_nodes[index_i].weights[j];
        }

        output_nodes[index_i].output += 1/(1+ exp(-1* output_nodes[index_i].output));
...
void *result_calculation(void *threadarg){
    struct result_thread_data *my_data;
    my_data = (struct result_thread_data *) threadarg;

    
    int predictedNum = getNNPrediction();


    if (predictedNum!=*(my_data->lbl)) (*(my_data->errCount))++;

    printf("\n      Prediction: %d   Actual: %d ",predictedNum, *(my_data->lbl));

    displayProgress(*(my_data->imgCount), *(my_data->errCount), 5, 66);
...
void testNN(){
    ...
    pthread_t threads_out[10];
    pthread_t threads_result[1];

    struct out_thread_data td_out[10];
    struct result_thread_data td_result[1];
    ...
    for (int imgCount=0; imgCount<MNIST_MAX_TESTING_IMAGES; imgCount++){
        ...
        sem_init(&lock_out_pass, 0, 1);
        sem_init(&lock_pass, 0, 1);
        ...
        struct out_thread_data *my_data;
        struct out_thread_data my_data2;
        pthread_t threads_hidden[num_hidden_threads];
        // loop through all hidden cells for the given image
        for( int i2 = 0; i2 < num_hidden_threads; i2++ ) {
          sem_wait(&lock_out_pass);
          int* ptr1 = (int*)malloc(sizeof(int));
          *ptr1 = i2;
          hidden_thread_data *parg = (hidden_thread_data *) malloc(sizeof(hidden_thread_data));
          parg->img = &img;
          parg->thread_id = *ptr1;
          free(ptr1);
          sem_post(&lock_out_pass);
          int rc = pthread_create(&threads_hidden[i2], NULL, hidden_calculation, (void *)parg);
          if (rc) {
        ...
        for( int i = 0; i < num_hidden_threads; i++ ) {
            pthread_join(threads_hidden[i], NULL);
        }
        ...
        for( int i = 0; i < 10; i++ ) {
          sem_wait(&lock_pass);
          out_thread_data *parg = (out_thread_data *) malloc(sizeof(out_thread_data));
          parg->img = &img;
          parg->thread_id = i;
          td_out[i].img = &img;
          td_out[i].thread_id = i;
          sem_post(&lock_pass);
          int rc = pthread_create(&threads_out[i], NULL, output_calculation, (void *)parg);
          if (rc) {
             cout << "Error:unable to create thread," << rc << endl;
             exit(-1);
          }
        ...
        for( int i = 0; i < 10; i++ ) {
            pthread_join(threads_out[i], NULL);
        }


        //result thread
        for( int i = 0; i < 1; i++ ) {
          td_result[i].errCount = &errCount;
          td_result[i].imgCount = &imgCount;
          td_result[i].thread_id = i;
          td_result[i].lbl = &lbl;
          int rc = pthread_create(&threads_result[i], NULL, result_calculation, (void *)&td_result[i]);
          if (rc) {
             cout << "Error:unable to create thread," << rc << endl;
             exit(-1);
          }
    ...
    sem_destroy(&lock_out_pass);
    sem_destroy(&lock_pass); 

}
int main(int argc, const char * argv[]) {

    cout << "enter the number of hidden threads : ";

    cin >> num_hidden_threads;
